...
@WebServlet("/insourcing")
public class insourcing extends HttpServlet {
...
response.setContentType("text/html");
		...
		try {
		    ...
		    ps = conn.prepareStatement("select p.product_image, p.product_name, s.product_id, s.quantity, s.type, s.date, u.fname, u.lname\r\n"
		    		...
		    		+ "join users u on u.id = s.user_id where s.type = 'buy'\r\n"
		    ...
		    request.getRequestDispatcher("insourcing.jsp").forward(request, response);
		} catch (Exception e) {
